...
BEGIN_EVENT_TABLE(dlgSearchObject, pgDialog)
	...
	EVT_LIST_ITEM_RIGHT_CLICK(XRCID("lcResults"), dlgSearchObject::OnSelSearchResult)
...
dlgSearchObject::dlgSearchObject(frmMain *p, pgDatabase *db, pgObject *obj)
{
	...
	lcResults->InsertColumn(0, _("Schema"));
	lcResults->InsertColumn(1, _("Type"));
	lcResults->InsertColumn(2, _("Name"));
	lcResults->InsertColumn(3, _("Path"));
...
void dlgSearchObject::OnSearch(wxCommandEvent &ev)
{
	...
	if(set)
	{
		...
		while(!set->Eof())
		{
			...
			wxString ItemPath; 
			wxString objectSchema = set->GetVal(wxT("nspname"));
			...
			lcResults->SetItem(i, 0, objectSchema);
			lcResults->SetItem(i, 1, locTypeStr);
			lcResults->SetItem(i, 2, objectName);
			lcResults->SetItem(i, 3, TranslatePath(ItemPath));
			set->MoveNext();
			i++;
		}
		delete set;
	...
	if(lcResults->GetItemCount() > 0)
	{
		...
		lcResults->SetColumnWidth(3, wxLIST_AUTOSIZE);
	}
